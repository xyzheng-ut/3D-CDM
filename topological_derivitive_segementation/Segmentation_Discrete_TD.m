...
% Fixed point algorithm
        [GT, ind] = min(DT, [], 4);
        gradconfC = reshape(GT, n*m*l, 1);
        indCol = reshape(ind, n*m*l, 1);
...
% computation.
	old = reshape((v-u).*(v-u), n, m, l, 1); 
% DT initially zero.
	DT = zeros(n, m, l, length(cls)); 
	DistanceVariation = zeros(n, m, l, length(cls)); 
...
% Compute the cost function value.
	fd = rho * sum(sum(sum(old)));
	fb = (1-rho) * sum(sum(sum(nNeighs)));
	fc = fb + fd;
...
% Neighbours with the same class for each class.
	nNeighsCls = zeros(n, m, l, length(cls));
% Look to the front.
	tmp = 2*max(max(max(abs(u))))*ones(size(v)); 
	tmp(:,:,1:l-1) = u(:,:,2:l); 
% For each class.
	c = 1;
	for i = cls
...
% AdjustClassesValues Adjust classes values in each iteration.
    ...
    for c = co
        fcvd = inf; fcvi = inf;
        cl = c; ci = c;
        fcvo = sum(sum(sum((us-v).*(us-v))));
        cl =  min(min(min(us)));
        if c > min(min(min(us)))-1
            cl =  c - 1;
            tmp = us;
            I = find(tmp==c);
            tmp(I) = cl;
            fcvd = sum(sum(sum((tmp-v).*(tmp-v))));
        end
        ci =  max(max(max(us)));
        if c < max(max(max(us)))+1
            ci =  c + 1;
            tmp = us;
            I = find(tmp==c);
            tmp(I) = ci;
            fcvi = sum(sum(sum((tmp-v).*(tmp-v))));
        end
